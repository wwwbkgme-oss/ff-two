...
/// A development task claimed and executed by an agent.
...
pub struct Task {
    ...
    /// Agent consensus votes: agent_id → approved.
...
impl Task {
    pub fn new(
        ...
        let now = Utc::now();
        Self {
            id: Uuid::new_v4(), project_id, title: title.into(), description: description.into(),
    ...
    /// Returns `true` when at least `threshold` positive votes are present.
    pub fn has_consensus(&self, threshold: usize) -> bool {
        self.consensus.values().filter(|&&v| v).count() >= threshold
    }
}

/// Record of an agent claiming a task from the queue.
#[derive(Debug, Clone, Serialize, Deserialize)]
