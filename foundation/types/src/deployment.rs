...
impl std::fmt::Display for DeploymentEnv {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}", match self {
            Self::Staging => "staging", Self::Production => "production", Self::Preview => "preview",
        })
    }
}
...
impl Deployment {
    pub fn new(project_id: Uuid, env: DeploymentEnv, version: impl Into<String>) -> Self {
        let now = Utc::now();
        Self {
            id: Uuid::new_v4(), project_id, env,
            status: DeploymentStatus::Pending, version: version.into(),
            commit_sha: None, approvals: Vec::new(), logs: Vec::new(),
            url: None, created_at: now, updated_at: now, deployed_at: None,
        }
    }
}
